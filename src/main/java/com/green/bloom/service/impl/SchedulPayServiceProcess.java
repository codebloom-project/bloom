...
import com.green.bloom.mybatis.mapper.SchedulePayMapper;
...
@Service
public class SchedulPayServiceProcess implements SchedulePayService{
	
	private final SchedulePayMapper schedulePayMapper;
	
	@Override
	public void saveProcess(SchedulePaySaveDTO dto, Authentication auth) {
		// board테이블에 dto의 내용을 저장
		//log.debug(dto.toString());
		EmployeeDetails emp=(EmployeeDetails)auth.getPrincipal();
		dto.setEmpNo(emp.getEmpNo());
		schedulePayMapper.save(dto);
	...
	public void listProcess(int page, Model model) {
		...
		List<SchedulePayDTO> result=schedulePayMapper.findAllLimit(offset, limit);
		model.addAttribute("scheduleList", result);
		
		int rowCount = schedulePayMapper.countAll();
	...
	@Override
	public void updateProcess(SchedulePayUpdateDTO dto) {
		schedulePayMapper.updateScheduleCurrent(dto);
	...
	public void listMyProcess(int page, Model model, long empNo) {
		...
		int limit=10; // 한페이지에 표현할 rows수
		//RowBounds pagging=new RowBounds(offset, limit);
		List<SchedulePayDTO> result=schedulePayMapper.findLimit(offset, limit, empNo);
		model.addAttribute("scheduleList", result);
		
		int rowCount = schedulePayMapper.count(empNo);
		PageUtil pageUtil=PageUtil.createPage(rowCount, limit, page, 5);
		model.addAttribute("pu", pageUtil);
