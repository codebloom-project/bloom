...
import com.green.bloom.weather.dto.day.ItemData;
import com.green.bloom.weather.dto.day.WeatherDayResponseDTO;
...
public class WeatherDayController {
	...
	public List<ItemData> getUltraSrtNcst(WeatherDayInfo dto, Model model) throws IOException {
        ...
        WeatherDayResponseDTO result = objectMapper.readValue(sb.toString(), WeatherDayResponseDTO.class);
        //model.addAttribute("daylist", result.getResponse().getBody().getItems().getItem());
        //model.addAttribute("localDateTime", LocalDateTime.now());
        System.out.println("resultDay: "+result.getResponse().getBody().getItems().getItem());
        return result.getResponse().getBody().getItems().getItem().stream()
        		.filter(itemData->itemData.getCategory().equals("T1H")|| itemData.getCategory().equals("SKY"))
